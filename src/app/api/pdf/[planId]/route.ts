...
// Returns a print-optimized HTML agreement for a payment plan. Local dev ships
// HTML; production upgrade swaps in Cloudflare Browser Rendering (Puppeteer)
// to emit a real PDF.
export async function GET(
  ...
  const monthly = (row.monthly_cents / 100).toLocaleString('en-CA', {
    style: 'currency',
    currency: 'CAD',
  });
  const total = (row.total_cents / 100).toLocaleString('en-CA', {
    style: 'currency',
    currency: 'CAD',
  });
