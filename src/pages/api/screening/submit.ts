...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  let reasons: string[];
  try {
    const parsed = reasonsSchema.parse(JSON.parse(String(form.get('reasons') ?? '[]')));
    reasons = parsed;
  } catch {
...
function computePriority(args: { ticket: { amount_cents: number; offence_code: string }; reasons: string[] }): number {
